    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-6 p-4 max-w-4xl">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Customer Info -->
                <div class="rounded-lg border border-sidebar-border p-4 space-y-2">
                    <h2 class="font-semibold text-sm text-muted-foreground uppercase tracking-wide">Customer</h2>
                    <p class="font-medium">{{ order.customer.user.name }}</p>
                    <p class="text-sm text-muted-foreground">{{ order.customer.user.email }}</p>
                    <p v-if="order.customer.phone" class="text-sm text-muted-foreground">{{ order.customer.phone }}</p>
                </div>

                <!-- Shipping Address -->
                <div class="rounded-lg border border-sidebar-border p-4 space-y-2">
                    <h2 class="font-semibold text-sm text-muted-foreground uppercase tracking-wide">Shipping Address</h2>
                    <p class="font-medium">{{ order.shipping_name }}</p>
                    <p class="text-sm">{{ order.shipping_address_line1 }}</p>
                    <p v-if="order.shipping_address_line2" class="text-sm">{{ order.shipping_address_line2 }}</p>
                    <p class="text-sm">{{ order.shipping_city }}, {{ order.shipping_state }} {{ order.shipping_postcode }}</p>
                    <p class="text-sm text-muted-foreground">{{ order.shipping_country }}</p>
                </div>
            </div>
            ...
            <div class="rounded-lg border border-sidebar-border overflow-hidden">
                <div class="px-4 py-3 border-b border-sidebar-border bg-muted/50">
                    <h2 class="font-semibold text-sm text-muted-foreground uppercase tracking-wide">Items</h2>
                </div>
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-sidebar-border">
                        <tr v-for="item in order.items" :key="item.id" class="hover:bg-muted/30">
                            <td class="px-4 py-3 font-medium">{{ item.product_name }}</td>
                            <td class="px-4 py-3 text-muted-foreground font-mono text-xs">{{ item.product_sku ?? '—' }}</td>
                            <td class="px-4 py-3 text-right">${{ formatPrice(item.unit_price) }}</td>
                            <td class="px-4 py-3 text-right">{{ item.quantity }}</td>
                            <td class="px-4 py-3 text-right font-medium">${{ formatPrice(item.subtotal) }}</td>
                        </tr>
                    </tbody>
                </table>

                <!-- Totals -->
                <div class="border-t border-sidebar-border px-4 py-3 space-y-1 bg-muted/20">
                    ...
                    <div class="flex justify-between font-semibold pt-1 border-t border-sidebar-border">
                        <span>Total</span>
