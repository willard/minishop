...
import { index, create, edit, destroy } from '@/actions/App/Http/Controllers/Admin/CouponController';
        ...
        <div class="flex flex-col gap-6 p-4">
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-muted-foreground">{{ coupons.total }} total coupon{{ coupons.total === 1 ? '' : 's' }}</p>
                </div>
                ...
                <table class="w-full text-sm">
                    <thead class="bg-muted/50 text-muted-foreground">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left font-medium">Min. Order</th>
                    ...
                    <tbody class="divide-y divide-sidebar-border">
                        <tr v-if="coupons.data.length === 0">
                            <td colspan="8" class="px-4 py-8 text-center text-muted-foreground">
                                No coupons yet.
                                <Link :href="create().url" class="text-primary underline ml-1">Create your first coupon</Link>
                            </td>
                        </tr>
                        <tr
                        ...
                        >
                            <td class="px-4 py-3 font-mono font-semibold tracking-wide">{{ coupon.code }}</td>
                            <td class="px-4 py-3 capitalize text-muted-foreground">{{ coupon.type }}</td>
                            <td class="px-4 py-3 font-medium">{{ formatValue(coupon) }}</td>
                            <td class="px-4 py-3 text-muted-foreground">{{ formatMinOrder(coupon) }}</td>
                            <td class="px-4 py-3 text-muted-foreground">{{ coupon.expiry_date ?? '—' }}</td>
                            <td class="px-4 py-3 text-muted-foreground">{{ usageLabel(coupon) }}</td>
                            <td class="px-4 py-3">
