    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-6 p-4 max-w-4xl">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-semibold">{{ customer.user?.name ?? '—' }}</h1>
                    <p class="text-sm text-muted-foreground">{{ customer.user?.email }}</p>
                </div>
                <Badge :variant="customer.is_active ? 'default' : 'secondary'" class="ml-auto">
                    {{ customer.is_active ? 'Active' : 'Inactive' }}
                </Badge>
            </div>

            <!-- Customer Details -->
            <div class="rounded-lg border border-sidebar-border p-4 space-y-3">
                <h2 class="font-semibold text-sm text-muted-foreground uppercase tracking-wide">Contact Information</h2>
            ...
            <div class="rounded-lg border border-sidebar-border overflow-hidden">
                <div class="px-4 py-3 border-b border-sidebar-border bg-muted/50">
                    <h2 class="font-semibold text-sm text-muted-foreground uppercase tracking-wide">Recent Orders</h2>
                </div>
                <div v-if="customer.orders.length === 0" class="px-4 py-8 text-center text-muted-foreground text-sm">
                    No orders yet.
                </div>
                <table v-else class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-sidebar-border">
                        ...
                        >
                            ...
                            <td class="px-4 py-3 text-muted-foreground">{{ order.items.length }}</td>
